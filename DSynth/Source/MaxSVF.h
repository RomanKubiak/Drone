#ifndef MAXSVF_H_INCLUDED
#define MAXSVF_H_INCLUDED
...
public:
	...
	inline double setSampleRate(const double sr)
	{
		sampleRate = sr;
	}
private:
    inline void setParams(double _freq, double _res) {
        freq = _freq;
        res = _res;
        g = tan(double_Pi * freq / sampleRate);
